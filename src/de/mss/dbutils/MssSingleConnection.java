...
public class MssSingleConnection implements Connection {
   ...
   public MssResultSet executeQuery(String logId, String query) throws MssException {
      connect();
      try {
   ...
   public MssResultSet executeQuery(String logId, PreparedStatement stmt) throws MssException {
      if (stmt == null)
         throw new MssException(ErrorCodes.ERROR_INVALID_PARAM, "The Statement is null");

      MssResultSet result = null;
      this.busy = true;
      this.usedCount++ ;
      getLogger().debug(Tools.formatLoggingId(logId) + "Executing Query " + stmt);
      StopWatch s = new StopWatch();
      try {
         result = new MssResultSet(getLogger(), logId, stmt.executeQuery());
